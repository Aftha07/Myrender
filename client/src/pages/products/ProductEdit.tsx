...
export default function ProductEdit({ params }: ProductEditProps) {
  ...
  useEffect(() => {
    if (product) {
      form.reset({
        ...
        tax: product.tax || "Vat 15%",
        unit: product.unit || "",
        barcode: product.barcode || "",
        allowNotification: product.allowNotification || false,
        minimumQuantity: product.minimumQuantity || 0,
        inventoryItem: product.inventoryItem || true,
        sellingProduct: product.sellingProduct || true,
        buyingProduct: product.buyingProduct || true,
        warehouse: product.warehouse || "Warehouse",
        salesAccount: product.salesAccount || "Sales",
        purchasesAccount: product.purchasesAccount || "Purchases",
      });
      if (product.image) {
  ...
  const listType = isService ? 'Service List' : 'Product List';
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Products-Services / {listType} / Edit {itemType}</h1>
              </div>
            </div>
          </div>
        </div>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                ...
                >
                  {updateMutation.isPending ? "Updating..." : `Update ${itemType}`}
                </Button>
              </div>
            </div>
          </form>
